...
// TAXONOMY VERSION
// ===========================================================================
// Bumped whenever the meaning or set of any canonical value changes (a power
// is added/renamed, a status changes, etc.). Captured into a document's
// version manifest at lock time so we always know which vocabulary produced
// a frozen artifact (the lawyer's evidentiary requirement). Date-stamped.

export const TAXONOMY_VERSION = "tx-poa-taxonomy-2026.05.29";
...
export const DOCUMENT_STATUSES = Object.freeze([
  ...
  // Sprint 5: active lifecycle states (the lawyer-blessed model)
  Object.freeze({ key: "locked_for_signing", displayName: "Locked for signing", tone: "info" }),
  Object.freeze({ key: "executed", displayName: "Executed", tone: "success" }),
  // Sprint 6 (post-attorney-review correction): replaces the previous
  // auto-applied "superseded" status. Surfaces a possible replacement
  // situation to the attorney for legal review; no legal status change
  // happens until the attorney acts.
  Object.freeze({
    key: "potential_replacement_review_required",
    displayName: "Attorney review required",
    tone: "warning",
  }),
]);

// Sprint 5: the active document lifecycle. New code uses ONLY these states.
// Legacy values in DOCUMENT_STATUSES above remain displayable for safety but
// are not part of the forward workflow.
export const ACTIVE_DOCUMENT_STATUSES = Object.freeze([
  "draft",
  "locked_for_signing",
  "executed",
  "revoked",
  // Sprint 6: detection-only intermediate state; an older POA sits here
  // when a newer one has been created but no express revocation exists.
  // Treated as still legally effective until attorney action.
  "potential_replacement_review_required",
  // "superseded" is retained as a value but is no longer part of the
  // active workflow. New code does not transition documents to this
  // status; it remains only for backwards compatibility with any
  // pre-existing rows.
  "superseded",
]);
