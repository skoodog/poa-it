...
export const documentStatusEnum = pgEnum("document_status", [
  ...
  // Sprint 6 (post-attorney-review correction): "superseded" in our previous
  // model auto-applied whenever a newer POA was created. That is legally
  // wrong: in Texas, executing a later durable POA does NOT revoke a prior
  // durable POA unless the later POA expressly says so. We never wrote the
  // auto-supersession code (the value was scaffolded but unreached), and
  // we now formally take the position that we will not. Instead, when the
  // system detects a possible replacement situation, the older POA is
  // moved to this status — meaning "the platform detected a potentially
  // replacing later POA was created, but the legal call about whether the
  // older POA is still effective requires an attorney." From here, an
  // attorney can either (a) execute a real revocation instrument
  // (status → "revoked"), or (b) dismiss the flag with a reason
  // (status returns to its prior active state, dismissal logged).
  "potential_replacement_review_required",
]);
...
export const documents = pgTable(
  "documents",
  {
    ...
    // to "revoked", this links to the revocation instrument that caused the
    // change. Null for documents that have not been formally revoked.
    ...
    // (Retained for backward compatibility; new code uses the
    // potential_replacement_review_required flow instead.)
    supersededByDocumentId: uuid("superseded_by_document_id"),
    // Sprint 6 — Attorney-attention flag. Set whenever the platform has
    // detected something that needs an attorney's eyes before the document
    // can proceed past draft. Two known cases at introduction:
    //   1. A newer POA was created for this client and this older one may
    //      need to be revoked (or may not — attorney's call).
    //   2. The document was created via the unauthenticated client intake
    //      path (actor="client"); attorney must review before locking.
    // attentionReason is a short machine-readable tag (e.g.
    // "newer_poa_exists" or "client_submitted_intake") plus a human note.
    attentionRequired: boolean("attention_required").default(false).notNull(),
    attentionReason: text("attention_reason"),
    // When attention is dismissed by an attorney (e.g., they decided the
    // newer POA does NOT replace this one), the dismissal is logged here
    // for audit purposes.
    attentionDismissedAt: timestamp("attention_dismissed_at"),
    attentionDismissedBy: uuid("attention_dismissed_by"),
    attentionDismissalReason: text("attention_dismissal_reason"),
    // Sprint 6 — Consumer one-time purchase fields. A consumer wizard
    // session reaches the paywall, the user pays $X via Stripe Checkout, and
    // the webhook records the payment here. All four are null until paid; a
    // free/B2B-originated document never populates them.
...
// ===========================================================================
// STRIPE WEBHOOK EVENTS (Sprint 6) — idempotency ledger
// ===========================================================================
// Stripe may deliver the same webhook event more than once (network retries,
// dashboard "resend"). Recording every received event by its Stripe id, and
// refusing to re-process one we've already handled, is the only correct way
// to make webhook-driven fulfillment idempotent. We persist the full payload
// so we have an audit trail of exactly what Stripe sent us — useful both for
// debugging and (later) for replaying missed handlers.
//
// This is a ledger, not a queue: rows are created at receipt, marked
// processed when handlers complete, and never deleted.
// ===========================================================================

export const stripeWebhookEvents = pgTable(
  "stripe_webhook_events",
  {
    // Stripe's event id (evt_...) — globally unique, so it's our PK and the
    // natural dedupe key.
    id: text("id").primaryKey(),
    type: text("type").notNull(), // e.g. "checkout.session.completed"
    livemode: boolean("livemode").notNull(), // false for test-mode events
    apiVersion: text("api_version"), // the Stripe API version that produced it
    payload: jsonb("payload").notNull(), // the full event object
    // Lifecycle: rows start with processedAt=null; handler success sets it;
    // handler failures leave it null and write the error so we can retry.
