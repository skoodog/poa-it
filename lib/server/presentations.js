...
export async function snapshotPoaForPresentation(documentId) {
  ...
  if (doc.wizardSessionId) {
    ...
    if (session?.answers) {
      principalNameSnapshot = session.answers.principalFullLegalName || "";
      agentNameSnapshot = session.answers.agentFullLegalName || "";
      successorAgentNameSnapshot = session.answers.successorAgentFullLegalName || "";
      originalPoaPowersGranted = session.answers.powersGranted || [];
      originalPoaExecutionMethod = session.answers.executionMethod || null;
      poaIsSpringingType = session.answers.effectiveDateType === "springing";
    }
  }
